  ...
  return (
    <main className="min-h-screen bg-[#03050a] text-white selection:bg-cyan-500/30 selection:text-white">
      ...
      <section id="top" className="relative h-screen w-full overflow-hidden">
        ...
        {/* ========================================== */}
        {/* HUD PANEL: AHORA SÍ, CRISTAL PURO NINJA */}
        {/* ========================================== */}
        <AnimatePresence>
          {isHudOpen && (
            <>
              {/* Overlay mucho más transparente para dejar ver a Sophie */}
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              {/* El Panel de Cristal (Glassmorphism Real) */}
              <motion.div
                initial={{ x: "100%" }}
                animate={{ x: 0 }}
                exit={{ x: "100%" }}
                transition={{ type: "spring", damping: 25, stiffness: 200 }}
                className="absolute right-0 top-0 bottom-0 z-40 w-full max-w-md border-l border-white/10 bg-white/[0.03] p-8 pt-32 shadow-[-20px_0_50px_rgba(0,0,0,0.3)] backdrop-blur-xl flex flex-col justify-center"
              >
                <button
                  onClick={() => setIsHudOpen(false)}
                  className="absolute top-24 right-8 flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-white/5 text-white/70 transition hover:bg-white/20 hover:text-white backdrop-blur-md"
                >
                  <X className="h-5 w-5" />
                </button>
                ...
                <h1 className="mt-4 text-4xl font-extrabold leading-[1.1] tracking-tight text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                  ...
                  <span className="text-transparent bg-clip-text bg-gradient-to-l from-cyan-300 to-blue-400 drop-shadow-[0_0_15px_rgba(6,182,212,0.4)]">tu negocio.</span>
                </h1>
                ...
                <div className="mt-10 flex flex-col gap-4">
                  <button
                    ...
                    className="group relative flex w-full items-center justify-center gap-3 border border-cyan-400/60 bg-cyan-500/20 px-6 py-4 text-sm font-mono tracking-widest text-cyan-100 backdrop-blur-md transition-all hover:bg-cyan-500 hover:text-slate-950 hover:shadow-[0_0_30px_rgba(6,182,212,0.6)]"
                  >
                  ...
                  <a
                    href="#servicios"
                    onClick={() => setIsHudOpen(false)}
                    className="flex w-full items-center justify-center rounded-none border border-white/20 bg-black/20 px-6 py-4 text-sm font-mono tracking-widest text-white/80 backdrop-blur-md transition hover:bg-white/10 hover:text-white"
                  >
                    EXPLORAR MÓDULOS
                  </a>
                </div>
