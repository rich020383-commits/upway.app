...
export const metadata = {
  title: 'Planes Upway Health — Para clínicas, IPS y EPS | Upway',
  description: 'Planes de voz AI para salud. Desde consultorios hasta EPS. Compara minutos, números simultáneos y precios transparentes.',
};
...
function PlanCard({ plan, featured = false }: { plan: typeof ALL_HEALTH_PLANS[number]; featured?: boolean }) {
  ...
  return (
    <div id={plan.id} className={`relative flex flex-col rounded-[24px] border p-6 transition-all hover:-translate-y-1 hover:shadow-xl ${featured ? 'border-[#1b5ed6] bg-gradient-to-br from-[#0f172a] to-[#1b3a5f] text-white shadow-2xl' : 'border-slate-200 bg-white shadow-md'}`}>
      {featured && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#1b5ed6] px-4 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white shadow-lg">
          Más popular
        </div>
      )}
      ...
      <div className={`mb-5 space-y-2 text-sm ${featured ? 'text-slate-200' : 'text-slate-600'}`}>
        <div className="flex items-center gap-2"><span>📞</span><span><strong>{plan.includedNumbers}</strong> número{plan.includedNumbers > 1 ? 'es' : ''}</span></div>
        <div className="flex items-center gap-2"><span>⏱️</span><span><strong>{plan.includedMinutes.toLocaleString('es-CO')}</strong> min/mes</span></div>
        <div className="flex items-center gap-2"><span>🔀</span><span>Hasta <strong>{plan.concurrentCalls}</strong> simultáneas</span></div>
        {plan.overageCOP > 0 && <div className="flex items-center gap-2"><span>📈</span><span>Overage: <strong>${plan.overageCOP.toLocaleString('es-CO')} COP/min</strong></span></div>}
        <div className="flex items-center gap-2"><span>💾</span><span>Grabación: <strong>{plan.recordingRetention}</strong></span></div>
      </div>

      <ul className="mb-6 flex-1 space-y-2">
        {plan.features.map((f, i) => (
          <li key={i} className={`flex items-start gap-2 text-sm ${featured ? 'text-slate-200' : 'text-slate-600'}`}>
            <span className="mt-0.5 text-emerald-500">✓</span><span>{f}</span>
          </li>
        ))}
      </ul>
      ...
      {plan.requiresTelnyxApproval && (
        <div className={`mt-4 rounded-xl p-3 text-xs ${featured ? 'bg-white/10 text-slate-200' : 'bg-amber-50 text-amber-700'}`}>
          ⚡ Requiere ampliación de capacidad (lo tramita Upway)
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <section className="mx-auto max-w-6xl px-4 pb-12 pt-20 text-center">
        ...
        <h1 className="mx-auto max-w-3xl text-4xl font-black tracking-[-0.04em] text-slate-900 md:text-5xl">
          Planes para cada tipo de atención en salud
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-lg text-slate-500">
          Desde un consultorio hasta una red de EPS. Voz AI con WhatsApp, agenda, triaje y escalamiento. Sin letra pequeña.
        </p>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-16">
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {standardPlans.map((plan) => (
            <PlanCard key={plan.id} plan={plan} featured={plan.id === 'clinica-pro-1800'} />
          ))}
        </div>
        ...
        {customPlans.length > 0 && (
          <div className="mt-12">
            ...
            <p className="mx-auto mb-8 max-w-xl text-center text-sm text-slate-500">
              Para volúmenes que superan los planes estándar. Cotizamos a medida según tu operación.
            </p>
      ...
      <section className="bg-gradient-to-br from-[#0f172a] to-[#132642] py-16">
        <div className="mx-auto max-w-3xl px-4 text-center">
          ...
          <p className="mt-3 text-slate-300">Empieza hoy. Sin contratos largos, sin complicaciones.</p>
          <Link href="/login?segment=health" className="mt-8 inline-block rounded-full bg-white px-8 py-4 text-sm font-bold text-[#0f172a] transition-all hover:-translate-y-0.5 hover:shadow-xl">
            Comenzar ahora →
          </Link>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-200 py-8 text-center text-xs text-slate-400">
        Upway Business Group S.A.S · Bogotá, Colombia
      </footer>
    </div>
  );
}
