...
const getInitialIdioma = (): Idioma => {
  try {
    return (localStorage.getItem('upway_idioma') as Idioma) || 'es';
  } catch {
    return 'es';
  }
};
...
export function LanguageProvider({ children }: { children: ReactNode }) {
  const [idioma, setIdioma] = useState<Idioma>(getInitialIdioma);

  useEffect(() => {
    if (idioma !== 'es') return; // Ya se leyó del storage; solo detectar si nunca se configuró
    let saved: string | null = null;
    try {
      saved = localStorage.getItem('upway_idioma');
    } catch {
      return;
    }
    if (saved) return; // Ya había elección guardada, no hacemos nada
    {
